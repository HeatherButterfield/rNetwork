  ...
  methods: {
    getMessageCount() {
      var self = this;
			$.ajax({
				method: "GET",
				url: '/api/appuser/messages/count?token=' + this.data.appUserToken,
    ...
    getMessages() {
      var self = this;
			$.ajax({
				method: "GET",
				url: '/api/appuser/messages?token=' + this.data.appUserToken + '&skip=' + this.skip + '&take=' + this.take,
        headers: { "RequestVerificationToken": this.requestToken, "token": this.data.appUserToken },
				processData: false,
				contentType: 'application/json',
				dataType: 'json',
