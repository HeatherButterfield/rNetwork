new window.Vue({
  ...
  data: () => ({
    data: data,
    firstName: data.appUser.firstName,
    lastName: data.appUser.lastName,
  ...
  methods: {
      ...
      console.log(this.data.appUserToken);
			$.ajax({
				method: "PUT",
				url: '/api/appuser/name/update',
        headers: { "RequestVerificationToken": this.token, "token": this.data.appUserToken },
				processData: false,
				contentType: 'application/json',
				dataType: 'json',
        data: JSON.stringify({FirstName: this.firstName, LastName: this.lastName}),
