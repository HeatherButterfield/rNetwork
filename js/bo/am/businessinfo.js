new window.Vue({
  ...
  data: () => ({
    data: data,
    company: data.appUser.company,
    businessID: data.appUser.businessID,
  ...
  methods: {
      ...
      console.log(this.data.appUserToken);
			$.ajax({
				method: "PUT",
				url: '/api/appuser/business/update',
        headers: { "RequestVerificationToken": this.token, "token": this.data.appUserToken },
				processData: false,
				contentType: 'application/json',
				dataType: 'json',
        data: JSON.stringify({Company: this.company, BusinessID: this.businessID}),
